...
function Compare(){
    ...
    const {isOpen, compareList, compareBy} = useSelector((state)=>state.compare)
    ...
    if(isOpen){
    return <div className="nonVisibleDiv">
         <div className="modal-cart">
            <span className="modal-cart-close" onClick={()=>{
            ...
            <table className="tableCompare">
                ...
                {comparedItems.length > 0 ? comparedItems.map((item)=>{
                    return <CompareItem key={item.id} item={item} compare={compareBy}/>
